...
void commanderTask(void* param)
{
    ...
    while (1) {
        ...
        if (configParam.flight.ctrl == ALTHOLD_MODE
            || configParam.flight.ctrl == THREEHOLD_MODE) /*定高模式 和定点模式*/
        {
        ...
        if (radioinkConnectStatus() == true && getFlightPowerStatus() == true) {
            ...
            rcdata.thrust = flydata.thrust * 500;
            rcdata.pitch  = percent.pitch * 500;
            rcdata.roll   = percent.roll * 500;
            rcdata.yaw    = percent.yaw * 500;
            memcpy(p.data, &rcdata, p.dataLen);
            usblinkSendPacket(&p);
        }
    }
...
/*获取飞控数据*/
joystickFlyf_t getFlyControlData(void) { return flydata; }
